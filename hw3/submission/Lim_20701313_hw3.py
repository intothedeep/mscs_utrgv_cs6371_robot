...
def lowPassFilter(lpfw, lpfh):
    ...
    lowPass = np.ones((lpfw, lpfh)) / (lpfw * lpfh)
...
def conv(image, filter):
    ...
    for i in range(iw - fw + 1):
        for j in range(ih - fh + 1):
            subImage = image[i:i+fw, j:j+fh]
            out[i, j] = np.sum(subImage * filter)

    # Return the resulting image "out"
    return out

img = mpimg.imread("original.jpg")
# original.jpg (512x512) is already single-channel, but mpimg.imread can
# return a 3-channel array for other images, which breaks the template's
# `iw, ih = image.shape` in conv(). Guard by averaging channels to grayscale.
if img.ndim == 3:
    img = img.mean(axis=2)
imshow(img)
plt.show()
